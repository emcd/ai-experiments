...
def generate( components, layout_, component_name ):
    entry = layout_[ component_name ]
    elements = [ ]
    for element_name in entry.get( 'contains', ( ) ):
        elements.append( generate( components, layout_, element_name ) )
...
def prepare( vectorstores ):
    from types import SimpleNamespace
    from .callbacks import register as register_callbacks
    from .layouts import layout
    from .state import populate
    gui = { }
    dashboard = generate( gui, layout, 'dashboard' )
    gui = SimpleNamespace( **gui )
    populate( gui, vectorstores )
